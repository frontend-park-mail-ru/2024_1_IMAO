// taken from https://jsbin.com/gesive/edit?html,js,console,output
var canvas, context, colPicker;
...
function init() {
  ...
  var colPicker = document.getElementById('colore');

  // event to react to color change
  colPicker.addEventListener("input", function() {
  ...
  for (i = 0; i < values.length; i++) {
    ...
    // style it
    input.setAttribute('class', 'slide1');
...
function makeHistogram(x, y, width, height, values) {
  //console.log('makeHistogram() called: ', x, y, width, height, values)
  context.save();
  ...
  //console.log('rectWidth: ', rectWidth, 'vStep: ', vStep)

  //Set fill color to red and stroke color to black.
  var barFill = document.getElementById('colore').value;
